...
const Navbar: React.FC = () => {
  ...
  const isActive = (path: string) =>
    location.pathname === path ? 'text-emerald-700 font-semibold' : 'text-slate-600 hover:text-emerald-700';

  return (
    <nav className="bg-white/95 border-b border-slate-200 sticky top-0 z-50 backdrop-blur">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center gap-2">
            <Briefcase className="w-7 h-7 text-emerald-700" />
            <span className="text-xl font-bold text-slate-900">Work Pool</span>
            <span className="hidden sm:inline text-xs bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">India</span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            {isAuthenticated ? (
              <>
                <Link to="/notifications" className="relative p-2 text-slate-600 hover:text-emerald-700">
                ...
                <button onClick={handleLogout} className="p-2 text-slate-500 hover:text-red-600" title="Logout">
      ...
      {isAuthenticated && (
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex z-50">
          <Link to="/" className="flex-1 flex flex-col items-center py-2 text-xs text-slate-600">
            <Home className="w-5 h-5" />Home
          </Link>
          <Link to="/tasks" className="flex-1 flex flex-col items-center py-2 text-xs text-slate-600">
            <Briefcase className="w-5 h-5" />Tasks
          </Link>
          <Link to="/tasks/new" className="flex-1 flex flex-col items-center py-2 text-xs text-emerald-700">
            <PlusCircle className="w-5 h-5" />Post
          </Link>
          <Link to="/notifications" className="relative flex-1 flex flex-col items-center py-2 text-xs text-slate-600">
            <Bell className="w-5 h-5" />
            {unreadCount > 0 && (
          ...
          <Link to="/profile" className="flex-1 flex flex-col items-center py-2 text-xs text-slate-600">
