...
for app in work-pool-user-service work-pool-task-service work-pool-notification-service work-pool-payment-service work-pool-rating-service work-pool-api-gateway; do
  ...
  echo "$app containers: $all_containers"
  echo "$all_containers" | grep -q istio-proxy
...
ui_init_containers=$(kubectl -n "$NS" get pod -l app=work-pool-ui -o jsonpath='{.items[0].spec.initContainers[*].name}')
ui_all_containers="$ui_containers $ui_init_containers"
echo "work-pool-ui containers: $ui_all_containers"
if echo "$ui_all_containers" | grep -q istio-proxy; then
...
for i in $(seq 1 20); do
  curl -fsS -H 'Host: api.work-pool.org' http://127.0.0.1:18080/actuator/health >/dev/null
  curl -fsS -H 'Host: api.work-pool.org' http://127.0.0.1:18080/api/v1/tasks >/dev/null || true
done
